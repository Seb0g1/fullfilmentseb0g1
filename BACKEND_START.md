# 🚀 Запуск Backend сервера

## Проблема: ERR_CONNECTION_REFUSED

Если вы видите ошибку `ERR_CONNECTION_REFUSED` в консоли браузера, это означает, что **backend сервер не запущен**.

## Быстрое решение

### Windows:

1. **Запустите скрипт:**
   ```cmd
   run-backend.bat
   ```

2. **Или вручную:**
   ```cmd
   cd server
   npm install
   node server.js
   ```

### Linux/Mac:

```bash
cd server
npm install
node server.js
```

## Первая настройка

При первом запуске создайте файл `server/.env`:

```env
TELEGRAM_BOT_TOKEN=ваш_токен_бота
TELEGRAM_GROUP_CHAT_ID=ваш_chat_id
TELEGRAM_TOPIC_CHAT_CLIENT=0
TELEGRAM_TOPIC_CALCULATOR=0
TELEGRAM_TOPIC_CONTACT_FORM=0
PORT=3001
NODE_ENV=development
```

## Проверка работы

После запуска вы должны увидеть:
```
🚀 Chat server running on http://localhost:3001
📱 Telegram bot: ✅ Configured (или ❌ Not configured)
```

Откройте в браузере: http://localhost:3001/health

Должен вернуться JSON:
```json
{
  "status": "ok",
  "telegramConfigured": true
}
```

## Запуск для разработки

Для разработки нужно запустить **два процесса**:

1. **Backend сервер** (в одном терминале):
   ```cmd
   run-backend.bat
   ```

2. **Frontend dev-сервер** (в другом терминале):
   ```cmd
   npm run dev
   ```

## Запуск для production

Используйте PM2 или systemd (см. `DEPLOY.md`):

```bash
pm2 start ecosystem.config.js
```

## Устранение проблем

### Порт 3001 занят

```bash
# Windows
netstat -ano | findstr :3001

# Linux/Mac
lsof -i :3001
```

Остановите процесс или измените порт в `server/.env`:
```env
PORT=3002
```

И обновите `VITE_API_URL` в `.env`:
```env
VITE_API_URL=http://localhost:3002
```

### Зависимости не установлены

```bash
cd server
npm install
```

### Ошибка с Telegram ботом

- Проверьте токен в `server/.env`
- Убедитесь, что бот добавлен в группу
- Проверьте Chat ID (должен быть отрицательным числом для группы)

## Структура

```
server/
├── server.js          # Backend сервер
├── package.json       # Зависимости
├── .env              # Конфигурация (создайте сами)
└── node_modules/     # Установленные зависимости
```

